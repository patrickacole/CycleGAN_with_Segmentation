...
class Discriminator(tf.keras.Model):
    ...
    '''
    def __init__(self, output_nc, ngf, n_layers=3):
        super(Discriminator, self).__init__(name='discriminator')
        self.output_nc = output_nc
        self.model = tf.keras.Sequential()
        self.model.add(layers.Conv2D(filters=ngf, kernel_size=5, strides=2, padding='valid', use_bias=True, data_format='channels_last'))
        self.model.add(layers.LeakyReLU(alpha=0.2))
        for i in range(1, n_layers):
            mult = min(2 ** (i + 1), 8)
            self.model.add(layers.Conv2D(filters=ngf*mult, kernel_size=3, strides=2, padding='valid', use_bias=True, data_format='channels_last'))
